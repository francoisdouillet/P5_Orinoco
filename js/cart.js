...
const removeElementFromBasket = (name) => {
  ...
  const basketWithoutRemovedElement = baskets.filter(
    (elem) => elem.name !== name
  );
...
if (baskets == null || baskets.length === 0) {
  ...
  for (basket of baskets) {
    ...
    titleCard.textContent = basket.quantity + " x " + basket.name;
    ...
    price.textContent = basket.price / 100 + " €";
  ...
  form.innerHTML = `<form class="p-6 m-auto flex flex-col" name="userInput" id="myForm">
